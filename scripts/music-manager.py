...
class SolaraMusicManager:
    ...
    def run_ssh(self, cmd: str, timeout: int = 60) -> tuple:
        """执行 SSH 命令"""
        # 用双引号包裹命令，正确处理管道符和参数
        full_cmd = f'sshpass -p "{self.nas_pass}" ssh -o StrictHostKeyChecking=no {self.nas_host} "{cmd}"'
...
"""
    ...
    def download_to_nas(self, url: str, filepath: str) -> bool:
        ...
        self.run_ssh(f"mkdir -p '{dir_path}'")
        
        cmd = f"curl -s -L --max-time 180 '{url}' -o '{filepath}'"
        ...
        check_cmd = f"stat -c%s '{filepath}' 2>/dev/null || echo '0'"
        _, size_out, _ = self.run_ssh(check_cmd)
    ...
    def scrape_metadata(self, filepath: str, song_name: str, artist: str = "") -> bool:
        ...
        # 直接运行 Music Tag Web 的批量刮削脚本（自动处理所有文件）
        cmd = 'docker exec music-tag-web python /app/scrape_music.py'
        ...
        if code == 0 and ("✅" in stdout or "成功" in stdout or "歌词" in stdout):
            ...
            print(f"刮削输出：{stdout[:300]}{'...' if len(stdout) > 300 else ''}")
            return True
        else:
            print(f"❌ 刮削失败：{stderr[:200] if stderr else '未知错误'}")
            return False
